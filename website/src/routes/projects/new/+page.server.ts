...
export const load: PageServerLoad = async ({ locals }) => {
	...
	try {
		...
		hackatimeError = 'Could not load Hackatime projects. You can still create the project.';
	}

	return { hackatimeProjects, hackatimeError };
};
	...
	create: async ({ locals, request }) => {
		...
		if (hackatimeProjects.length > 0) {
			try {
				...
				// API unreachable — skip validation, allow submission
			}
		}
