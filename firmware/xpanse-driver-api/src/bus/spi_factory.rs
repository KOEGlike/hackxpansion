...
use crate::bus::allocator::SpiResource;
...
impl PioManager {
    ...
    pub fn create_pio_spi<CLK: PioPin, MOSI: PioPin, MISO: PioPin>(
        ...
        match (block, sm) {
            (0, 0) => {
                let pio = self.pio0.take().expect("PIO0 not available");
                let Pio { mut common, sm0, .. } = pio;
                Box::new(PioSpiBus::new(&mut common, sm0, clk, mosi, miso, config))
            }
            (1, 0) => {
                let pio = self.pio1.take().expect("PIO1 not available");
                let Pio { mut common, sm0, .. } = pio;
                Box::new(PioSpiBus::new(&mut common, sm0, clk, mosi, miso, config))
            }
            (2, 0) => {
                let pio = self.pio2.take().expect("PIO2 not available");
                let Pio { mut common, sm0, .. } = pio;
                Box::new(PioSpiBus::new(&mut common, sm0, clk, mosi, miso, config))
            }
            _ => panic!("Unsupported PIO block/sm: {}/{}", block, sm),
        }
    }
}

/// Build a `SpiBusHandle` without the hardware‑SPI path (PIO or bit‑bang only).
/// Works with any bank's pins — only requires `PioPin`.
pub fn create_spi_bus_pio(
...
/// Build a `SpiBusHandle` from a `SpiResource` and GPIO pins.
///
/// The pins must implement `PioPin` (for PIO fallback) and be convertible to
/// `AnyPin` (for bit‑bang fallback).  `BasePin` satisfies both.
pub fn create_spi_bus(
    resource: SpiResource,
    clk: Peri<'static, impl PioPin + embassy_rp::spi::ClkPin<embassy_rp::peripherals::SPI1>>,
    mosi: Peri<'static, impl PioPin + embassy_rp::spi::MosiPin<embassy_rp::peripherals::SPI1>>,
    miso: Peri<'static, impl PioPin + embassy_rp::spi::MisoPin<embassy_rp::peripherals::SPI1>>,
    config: spi::Config,
    pio_manager: &mut PioManager,
) -> SpiBusHandle {
    match resource {
