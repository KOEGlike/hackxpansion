...
pub async fn map_adc<'a>(adc: &mut adc::Adc<'a>, slot: ModuleSlot) -> Result<ModuleID, i2c::Error> {
    let (md0_reading, md1_reading) = match slot {
        ModuleSlot::FrontLeft => (adc.read_ch2().await?, adc.read_ch3().await?),
        ModuleSlot::FrontRight => (adc.read_ch0().await?, adc.read_ch1().await?),
        ModuleSlot::BackLeft => (adc.read_ch4().await?, adc.read_ch5().await?),
        ModuleSlot::BackRight => (adc.read_ch6().await?, adc.read_ch7().await?),
    };
